...
const lineNotifyToken = JSON.parse(fs.readFileSync("./settings.json", "utf8")).gyotokuLineNotifyToken;
...
(async () => {
  while(true){
	...
	availability = value.replace(/\n/g,"").replace(/.*([○×△]).*/g,"$1")
      ...
      // 通知
      if(akiarray.length){
        if(fs.existsSync("/tmp/gyotoku-court-previous.txt")){
          const previous = fs.readFileSync("/tmp/gyotoku-court-previous.txt","UTF-8")
          if(previous != JSON.stringify(akiarray)){
            const myLine = new Line();
            myLine.setToken(lineNotifyToken);
            myLine.notify(JSON.stringify(akiarray).toString());
            fs.writeFileSync("/tmp/gyotoku-court-previous.txt", JSON.stringify(akiarray))
          }
	} else {
          const myLine = new Line();
          myLine.setToken(lineNotifyToken);
