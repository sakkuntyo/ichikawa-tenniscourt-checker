...
const lineNotifyToken = JSON.parse(fs.readFileSync("./settings.json", "utf8")).hukueiLineNotifyToken;
...
(async () => {
  while(true){
	...
	availability = value.replace(/\n/g,"").replace(/.*([○×△]).*/g,"$1")
      ...
      // 通知
      if(akiarray.length){
        if(fs.existsSync("/tmp/hukuei-court-previous.txt")){
          const previous = fs.readFileSync("/tmp/hukuei-court-previous.txt","UTF-8")
          if(previous != JSON.stringify(akiarray)){
            const myLine = new Line();
            myLine.setToken(lineNotifyToken);
            myLine.notify(JSON.stringify(akiarray).toString());
            fs.writeFileSync("/tmp/hukuei-court-previous.txt", JSON.stringify(akiarray))
          }
	} else {
          const myLine = new Line();
          myLine.setToken(lineNotifyToken);
