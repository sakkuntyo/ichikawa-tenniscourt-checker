...
(async () => {
  while(true){
    ...
    try{
      ...
      if(akiarray.length){
        if(fs.existsSync("/tmp/graspo-court-previous.txt")){
          const previous = fs.readFileSync("/tmp/graspo-court-previous.txt","UTF-8")
          if(previous != JSON.stringify(akiarray)){
            const myLine = new Line();
            myLine.setToken(lineNotifyToken);
            myLine.notify(JSON.stringify(akiarray).toString());
            fs.writeFileSync("/tmp/graspo-court-previous.txt", JSON.stringify(akiarray))
          }
	} else {
          const myLine = new Line();
          myLine.setToken(lineNotifyToken);
